...
public class PinkSlug extends MonsterManager
{
    ...
    public void getDrops() throws InterruptedException
    {
        ...
        if(threshold < 50)
            Main.LootScreen.setLootMisc("Q-Cube",amt);
             threshold = (int)(Math.random() * 101);
             amt = (int)(Math.random() * 4) + 1;
        if(threshold < 25)
            Main.LootScreen.setLootMisc("U-Cube",amt);
            threshold = (int)(Math.random() * 101);
             amt = (int)(Math.random() * 10) + 5;
        if(threshold < 13)
            Main.LootScreen.setLootMisc("I-Cube",amt);
            threshold = (int)(Math.random() * 101);
             amt = (int)(Math.random() * 10) + 5;
        if(threshold < 6)
            Main.LootScreen.setLootMisc("N-Cube",amt);
            threshold = (int)(Math.random() * 101);
             amt = (int)(Math.random() * 10) + 10;
        if(threshold < 3)
            Main.LootScreen.setLootMisc("C-Cube",amt);
            threshold = (int)(Math.random() * 101);
        if(threshold < 2)
            Main.LootScreen.setLootMisc("Y-Cube",20);
            threshold = (int)(Math.random() * 101);
        if(threshold < 33)
