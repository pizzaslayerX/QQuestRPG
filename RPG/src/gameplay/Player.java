...
public class Player implements Serializable
{
    ...
    public static ArmorManager[] armorSet = new ArmorManager[3];
    ...
    private String[] itemSlot = new String[consumables.ItemManager.itemSlot.length];
    ...
    public static void load() throws InterruptedException
    {
      ...
      else
      {
       ...
       levelPoints = p.levelP;
        ...
        consumables.ItemManager.slotCount = p.slotCount;
        Ability.Actives.Manager.pageCount = p.abilityPageCount;
        if(consumables.ItemManager.slotCount == 4)
        	consumables.ItemManager.slot4unlock = true;
        else if(consumables.ItemManager.slotCount == 5)
        {
        	consumables.ItemManager.slot4unlock = true;
        	consumables.ItemManager.slot5unlock = true;
        }
        
       for(int i = 0;i<consumables.ItemManager.slotCount;i++)
       {
    ...
    public static void getItems() throws InterruptedException
    {
        ...
        int slot = consumables.ItemManager.fightPrintItems();
        MainFightPanel.clearDisplay();
        System.out.println(slot + "item slot chosen");
        if(slot != consumables.ItemManager.slotCount) {
        	consumables.ItemManager.useItems(slot+1);
        	 MainFightPanel.clearDisplay();
        }
    ...
    public static void getInventory() throws InterruptedException
    {
       ...
       while(leave == false)
       {
        ...
        run.append("\nArmor Set: " + armorSet[equippedArmor].name + " || Def: " + getDef() + " | " + armorSet[equippedArmor].desc);
    ...
    public static void switchWeapon() throws InterruptedException {
        	...
        	Story.userWait();
        	RPGRunner.output.setText("");
    	if(RPGRunner.battle == true && armorSet[equippedArmor].timeScore() < 3) {
    ...
    public static void switchArmor() throws InterruptedException {
        	...
        	if(choice == 1 || choice == 2 || choice == 3) {
        		...
        		if(armorSet[equippedArmor] == null) {
        			equippedArmor = 0;
        			return;
        		}
        	}
        	...
        	RPGRunner.append("\nYou equipped " + armorSet[equippedArmor].name + "\n");
        	Story.userWait();
        	RPGRunner.output.setText("");
        	if(RPGRunner.battle == true && armorSet[equippedArmor].timeScore() < 6) {
        		RPGRunner.pTurn = false;
        		RPGRunner.eTurn = true;
        	}
    ...
    public static int getShockR() {
    	return (armorSet[equippedArmor].shockR + baseShockR);
    }
    
    public static int getFireR() {
    	return (armorSet[equippedArmor].fireR + baseFireR);
    }
    
    public static int getIceR() {
    	return (armorSet[equippedArmor].iceR + baseIceR);
    }
    
    public static int getDef() {
    	return baseDef + armorSet[equippedArmor].def;
    }
